...
public class SystemController {
    ...
    public HttpEntity<byte[]> exportCVSAttendance(@PathVariable long courseId, @PathVariable long sectionId)
            throws IOException {
    ...
    public String uploadCSVFile(Model model, @RequestParam("file") MultipartFile file, HttpServletRequest request) {
        ...
        ArrayList<TeacherCourse> teacherCourses = new ArrayList<>(
                courseService.listCourseByTeacherAndSemester(teacher, semester));
    ...
    public String enrollStudent(Model model,
        ...
        Object recordsObj = session.getAttribute("records");
        if (recordsObj == null || !(recordsObj instanceof List<?>)) {
            // Handle error - redirect with error message
            model.addAttribute("message", "No valid records found in session");
            return "system/import-student";
        }

        @SuppressWarnings("unchecked")
        List<List<String>> records = (List<List<String>>) recordsObj;
